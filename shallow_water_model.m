...
for n=1:nt
  ...
  if(numerics==0) %FT (Euler)
    ...
    h(y,x)=h(y,x)+dt*rhs_h(u,v,h,nx,ny,dx,dy,nifad);
  ...
  elseif(numerics==1) %CT (leapfrog)
    ...
    u_2(y,x)=u_1(y,x)+2*dt*rhs_u(u,v,h,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    v_2(y,x)=v_1(y,x)+2*dt*rhs_v(u,v,h,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    h_2(y,x)=h_1(y,x)+2*dt*rhs_h(u,v,h,nx,ny,dx,dy,nifad);
    u_1=u; v_1=v; h_1=h;
    u=u_2; v=v_2; h=h_2;
  
  elseif(numerics==4) %RK4
    urhs1=rhs_u(u,v,h,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    vrhs1=rhs_v(u,v,h,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    hrhs1=rhs_h(u,v,h,nx,ny,dx,dy,nifad);
    u1=u; v1=v; h1=h;
    u1(y,x)=u(y,x)+0.5*dt*urhs1;
    v1(y,x)=v(y,x)+0.5*dt*vrhs1;
    h1(y,x)=h(y,x)+0.5*dt*hrhs1;
    urhs2=rhs_u(u1,v1,h1,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    vrhs2=rhs_v(u1,v1,h1,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    hrhs2=rhs_h(u1,v1,h1,nx,ny,dx,dy,nifad);
    u2=u; v2=v; h2=h;    
    u2(y,x)=u(y,x)+0.5*dt*urhs2;
    v2(y,x)=v(y,x)+0.5*dt*vrhs2;
    h2(y,x)=h(y,x)+0.5*dt*hrhs2;
    urhs3=rhs_u(u2,v2,h2,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    vrhs3=rhs_v(u2,v2,h2,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    hrhs3=rhs_h(u2,v2,h2,nx,ny,dx,dy,nifad);
    u3=u; v3=v; h3=h;
    u3(y,x)=u(y,x)+dt*urhs3;
    v3(y,x)=v(y,x)+dt*vrhs3;
    h3(y,x)=h(y,x)+dt*hrhs3;
    urhs4=rhs_u(u3,v3,h3,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    vrhs4=rhs_v(u3,v3,h3,nx,ny,dx,dy,g,f,nifcor,nifwind,nifad);
    hrhs4=rhs_h(u3,v3,h3,nx,ny,dx,dy,nifad);
